...
@ServerEndpoint(value = "/ws/news", configurator =HttpSessionConfigurator.class )
@Component
public class NewsSocketHander {
	...
	public void onopen(Session session, EndpointConfig config) {
		...
		new Thread() {
			public void run() {
				//初始数量
				int countInit =biz.Queryunreadinformation(new News(userid,null));
				while (true) {
					try {
						session.getBasicRemote().sendText("pong");
						Thread.sleep(1000);
						int count = biz.Queryunreadinformation(new News(userid,null));
						if (count > countInit) {
